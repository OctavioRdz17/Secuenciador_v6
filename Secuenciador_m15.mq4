//+------------------------------------------------------------------+
//|                                              Secuenciador_m15.mq4 |
//|                                   Copyright 2022, Toktrading.net |
//+------------------------------------------------------------------+
//Indicador de secuencia 
#property copyright "Copyright 2024, Octavio Rodriguez"
#property link      "www.josecantillofx.com/herramientas"
...
input   int PipsRectangulo = 35;//Altura de Rectangulos
   ...
   bool set_alarm  = false;//Alarmas Activadas 
   int start_alarm = 8; //Hora de inicio alarmas
   int end_alarm =  16; //Hora de fin alarmas
input   color ColorMaximo = clrDeepSkyBlue;
input   color ColorMinimo = clrCrimson;
   bool bodyLevels = false; //Secuencia de Cuerpos
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
      if(primerciclo==true)
      {
           maximo = elegirMaximo(bodyLevels,barrasIniciales);
           minimo = elegirMinimo(bodyLevels,barrasIniciales);
           maximoNuevo = maximo;
           minimoNuevo = minimo;
  ...
  bool revisandoRetroceso(int inicioScan,int finScan,double inicioTendencia,double finTendencia)
    {
      ...
      double retrocesoCompararOpen;
      double retrocesoCompararClose;
...
//////////////////////////////////////////////////////////////    
   ...
   int revisarMinimo(int inicioScan,int finScan)
    {
      ...
      int checkminOpen = iLowest(NULL,timeframe,MODE_OPEN,velascontar,inicioScan);
      int checkminClose = iLowest(NULL,timeframe,MODE_CLOSE,velascontar,inicioScan);
      int checkmin; 
        
        if(checkminOpen <= checkminClose){
            checkmin = checkminOpen;
        }else{
            checkmin = checkminClose;
        }
   ...
   int revisarMaximo(int inicioScan,int finScan)
    {
      int velascontar = finScan - inicioScan;
      int checkmaxOpen = iHighest(NULL,timeframe,MODE_OPEN,velascontar,inicioScan);
      int checkmaxClose = iHighest(NULL,timeframe,MODE_CLOSE,velascontar,inicioScan);
      int checkmax;

      if(checkmaxOpen >= checkmaxClose){
            checkmax = checkmaxOpen;
        }else{
            checkmax = checkmaxClose;
        }
      return (checkmax);
    }
...
double elegirMaximo(bool body, int shift){
    if(body){
        ...
        return iHigh(NULL,Period(),shift);
    }
}

double elegirMinimo(bool body, int shift){
    if(body){
        if(iOpen(Symbol(),Period(),shift) >= iClose(Symbol(),Period(),shift)){
            return iClose(NULL,timeframe,shift);
        }else{
...
void tendenciaCiclo(int perm)
{
    ...
    velaevaluandomax = elegirMaximo(bodyLevels,perm);
    velaevaluandomin = elegirMinimo(bodyLevels,perm); 
            ...
            if(tendenciaActual==1 && velaevaluandomax > maximoNuevo && velaevaluandomin >= minimo )
            {
              //busca retroceso para cambiar a tendencia 2
               if(velaevaluandomax>maximoPasado)
               {
                  ...
                  maximoPasado = 9999999;
                  minimo = elegirMinimo(bodyLevels,posicionMinimo);
              ...
              if(velaevaluandomin<minimoPasado)
               {
                  ...
                  maximo = elegirMaximo(bodyLevels,posicionMaximo);
